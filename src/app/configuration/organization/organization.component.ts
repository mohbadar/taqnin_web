...
export class OrganizationComponent implements OnInit {
	...
	addNew() {
		const modalRef = this.modalService.open(OrganizationCreateDialogComponent);
		modalRef.componentInstance.organizationCreateEventEmitter.subscribe((createdRecord) => {
	...
	viewRecord(recordId) {
		this.spinner.show();
		this.organizationService.getOrganizationById(recordId).subscribe(data => {
			console.log('you data has', data);
			const modalRef = this.modalService.open(OrganizationViewDialogComponent);
	...
	editRecord(recordId) {
		this.spinner.show();
		this.organizationService.getOrganizationById(recordId).subscribe(data => {
			console.log('the organization returned is', data);
			const modalRef = this.modalService.open(OrganizationEditDialogComponent);
			modalRef.componentInstance.data = data;
			modalRef.componentInstance.organizationEditEventEmitter.subscribe((updatedRecord) => {
	...
	{
		const modalRef = this.modalService.open(OrganizationDeleteDialogComponent);
		modalRef.componentInstance.data = recordId;
		modalRef.componentInstance.organizationDeleteEventEmitter.subscribe(() => {
		this.reloadData();
		});
	}
